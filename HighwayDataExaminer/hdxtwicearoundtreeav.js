...
var hdxTwiceAroundTreeAV = {
    ...
    //loop variable that tracks which vertex is currently being operated upon
    nextToCheck: -1,

    //stores the index of the waypoint that is selected by the user to be the start of every path
    startVertex: 0,

    //stores adjacency matrix for all vertices, for example graph[0][1] will give you the distance from vertex 0 to 1
    graph: null,

    //stores the permutation/path and the polyline corresponding to that path
    finalPath: [],
    finalPoly: [],

    //stores the polylines for the edges of the complete graph and its spanning tree
    edgePoly: [],

    //holds the length of the path so far
    currDistance: 0,

    //used to store the html which is printed into the AV control entry
    table: null,
    ...
    avActions : [
        {
            ...
            code: function(thisAV){
                ...
                //this section of code creates the complete graph and plots all the edges on the map

                //here we are developing the list of vertices and the 2D array that will store our polylines
                let arr = [];
                ...
                //this is where all the edges of the complete graph are constructed and then added to the map
                for(let i = 0; i < waypoints.length; i++){
                    for(let j = i + 1; j < waypoints.length; j++){
                        let currCoords = [[waypoints[i].lat, waypoints[i].lon],[waypoints[j].lat,waypoints[j].lon]];
                ...
                //here is the rainbow color generator
                ...
                //next we must generate the complete graph behind the scenes for us to perform the mst and dft algorithms
        ...
        },
        {
            ...
            code: function(thisAV){
                ...
                //here we perform prims on our complete graph so that we can perform our approximation on the minimum spanning tree
                ...
                for(let i = 0; i < thisAV.graph.v.length; i++){
                    for(let j = i + 1; j < thisAV.graph.v.length; j++){
                        if(!thisAV.graph.validEdge[i][j]){
                            ...
                            //if the graph says the edge is valid, then change i
                            thisAV.edgePoly[i][j].setStyle({color: visualSettings.spanningTree.color,
                                                            opacity: 0.8,
                                                            weight: 4})
                        }
                    }
...

        {
            ...
            code: function(thisAV){
                ...
                //now we perform depth-first traversal on the spanning to get our initial path
                //currently there is nothing that visualizes this, but that can be changed in the future
                thisAV.graph.dft(thisAV.startVertex, thisAV.graph.visited);

                //here we add the startVertex to the end in order to complete the path
                ...
                //here we create the top of the table which will be added to in the loop below
...

        {
            ...
            code: function(thisAV){
                ...
                //check condition, if have checked every point in the path then we are done
                if(thisAV.nextToCheck < thisAV.finalPath.length - 1 ){
                    ...
                    //if we still need to check additional points highlight it
                    updateMarkerAndTable(thisAV.finalPath[thisAV.nextToCheck], visualSettings.v1,
                        30, false);

                    updateAVControlEntry("v1","from: #" + thisAV.finalPath[thisAV.nextToCheck] + " " + waypoints[thisAV.finalPath[thisAV.nextToCheck]].label);
                    

                    //the reason why this condition does not check the last point is because we need to highlight both v1 and v2,
                    //so if we don't account for the last point we will get an out of bounds error
                    if(thisAV.nextToCheck < thisAV.finalPath.length - 2){
                    updateMarkerAndTable(thisAV.finalPath[thisAV.nextToCheck + 1], visualSettings.v2,
                        30,false);
                        updateAVControlEntry("v2","to: #" + thisAV.finalPath[thisAV.nextToCheck + 1] + " " + waypoints[thisAV.finalPath[thisAV.nextToCheck + 1]].label);
                    } else {
                        updateAVControlEntry("v2","to: #" + thisAV.finalPath[0] + " " +waypoints[thisAV.finalPath[0]].label);
                    }
...

        {
            ...
            code: function(thisAV){
                ...
                //here we get the coordinates of the endpoints of the edge we are about to add
                let currCoords = [];
                currCoords[0] = [waypoints[thisAV.finalPath[thisAV.nextToCheck]].lat,
                waypoints[thisAV.finalPath[thisAV.nextToCheck]].lon];

                currCoords[1] = [waypoints[thisAV.finalPath[thisAV.nextToCheck + 1]].lat,
                waypoints[thisAV.finalPath[thisAV.nextToCheck + 1]].lon];

                //here we construct the polyline for an edge used for the final path
                    let currEdge = L.polyline(currCoords, {
                    color: thisAV.currColor.color, //currColor comes from above generated by the rainbowGradiant
                    opacity: 0.6,
                    weight: 5
                    });

                //add to array to be removed later, adding polyline to map
                thisAV.finalPoly.push(currEdge);
                currEdge.addTo(map);

                updateMarkerAndTable(thisAV.finalPath[thisAV.nextToCheck], thisAV.currColor, 30, false);

                //calculating the distance of the current edge and then adding it to the total
                thisAV.currDistance = distanceInMiles(currCoords[0][0],currCoords[0][1],currCoords[1][0],currCoords[1][1]);
                ...
                thisAV.table += thisAV.hullTableRow()

                updateAVControlEntry('minPath',thisAV.table + '</tbody></table>');
            },
            logMessage: function(thisAV){
                return "Adding edge between vertex #" + thisAV.finalPath[thisAV.nextToCheck] + " and vertex #"
...

        {
            ...
            code: function(thisAV) {
                ...
                //make sure we clear any remaining parts graph and its spanning tree
                for(let i = 0; i < waypoints.length; i++){
                    for(let j = 0; j < waypoints.length; j++){
    ...
    hullTableRow() {
        ...
        return '<tr><td>' + this.finalPath[this.nextToCheck]+ ' &rarr; ' +  this.finalPath[this.nextToCheck + 1] + '</td><td>' + waypoints[this.finalPath[this.nextToCheck]].label +
            '</td><td>' + this.currDistance.toFixed(3) + 
            '</td></tr>';
    },
...
//this object is used to create the complete graph with all vertices in the graph. Verticies is an array of integers
function completeGraph(vertices){
    ...
    for(let i = 0; i <this.v.length; i++){
        for(let j = 0; j < this.v.length; j++){
            this.e[i][j] = distanceInMiles(waypoints[i].lat,waypoints[i].lon,waypoints[j].lat,waypoints[j].lon);
            this.validEdge[i][j] = false;
        }
    }

    //startVertex is a number, visited is an array of booleans, path is an array holding the sequence
    this.dft = function(startVertex){
        this.path.push(startVertex);
        this.visited[startVertex] = true;
        for(let i = 0; i < this.v.length; i++){
            if((!this.visited[i]) && (this.e[startVertex][i] > 1E-14 && this.e[startVertex][i] < Number.MAX_SAFE_INTEGER)){
                this.dft(i);
            }
        }
    }
    
    //if prune true, then remove non-valid edges from adjacency matrix
